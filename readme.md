...
SmartChef — это web-приложение, которое подбирает
персонализированные рецепты и меню на день с учётом калорийности, предпочтений и времени, которое пользователь готов потратить на готовку.


Проект реализован на собственном web-сервере на HttpListener с поддержкой маршрутизации, MVC и взаимодействия с PostgreSQL. Для получения рецептов использована Spoonacular API.



## Для запуска проекта
#### Создать файл appsettings.json, соответствующий файлу appsettings.template.json, добавив API ключ для подключения к Spoonacular API, username, password для подключения к Postgres.
```
{
  "ApiKey": "YOUR-API-KEY",
  "ConnectionString" : "Host=localhost;Username=username;Password=password;Database=postgres",
...
рекомендованная и ВО3, и Минздравом РФ для практического применения.
...
BMR умножают на коэффициент активности (PAL — Physical Activity Level):
...
## Главная страница - http://localhost:5000/meal_plan
